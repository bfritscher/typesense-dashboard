...
You can enable auto-login in several ways:

#### Option 1: Using Environment Variables (Recommended for Docker)

Set environment variables when running the container. The container will automatically generate `config.json` from these variables:
...
- `TYPESENSE_NODE_HOST` - Typesense server hostname (default: localhost)
- `TYPESENSE_NODE_PORT` - Typesense server port (default: 8108)
- `TYPESENSE_NODE_PROTOCOL` - Protocol: http or https (default: http)
- `TYPESENSE_NODE_PATH` - API path prefix (default: empty)
- `TYPESENSE_NODE_TLS` - Enable TLS: true or false (default: false)
- `TYPESENSE_HISTORY` - Optional JSON array of history entries
- `TYPESENSE_UI_CONFIG` - Optional JSON object for UI configuration

#### Option 2: Using config.json File

Map a config.json file to `/srv/config.json` in the container:

```bash
docker run -d -p 80:80 -v /path/to/config.json:/srv/config.json typesense-dashboard
```

**Note:** The application loads configuration from `config.json` at runtime. For Docker, this file is automatically generated from `TYPESENSE_*` environment variables when the container starts. For local development, you can manually create `public/config.json` or use the same environment variable approach.
